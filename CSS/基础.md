...
:target             //可用于选取当前活动的目标元素（URL 带有后面跟有锚名称 #，指向文档内某个具体的元素。这个被链接的元素就是目标元素(target element)。）
...
::selection         //被用户选取的选取的部分（只能向该选择器应用少量 CSS 属性：color、background、cursor 以及 outline）
```
#### 属性选择器
...
```
- 元素宽度 和 高度 = 内容的宽度 和 高度 + padding + border
...
- link 最大限度支持并行下载，@import 过多嵌套导致串行下载，出现 FOUC（浏览器样式闪烁或者叫做无样式内存闪烁）
